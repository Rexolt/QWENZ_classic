...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        main_layout.setSpacing(4)  # Kisebbre állítva a retró hatás miatt

        self.lbl_banner = QLabel("QWENZ")
        self.lbl_banner.setObjectName("bannerLabel")
    ...
    def add_songs(self):
        files, _ = QFileDialog.getOpenFileNames(
            self, "Zene hozzáadása", "", "Audio files (*.mp3 *.wav *.flac *.ogg *.aac)"
        )
        if files:
            old_count = self.audio_manager.media_count()
            for f in files:
